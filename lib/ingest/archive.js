...
async function openZipEntryStream(zipFile, entry) {
  ...
  if (entry.compressedSize === 0) {
    return pipelineZipEntry([Readable.from([])], createZipValidationStream(entry));
  }
  ...
  if (entry.method === 8) {
    ...
    streams.push(inflater);
  }
  return pipelineZipEntry(streams, createZipValidationStream(entry, compressedBytesConsumed));
}

function pipelineZipEntry(upstream, validation) {
  pipeline(...upstream, validation, (error) => {
    if (error && !validation.destroyed) validation.destroy(error);
  });
  return validation;
}
