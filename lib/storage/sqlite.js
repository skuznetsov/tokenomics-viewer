...
function createSqliteBackend(dependencies = {}) {
  ...
  function openTokenomicsDatabase(dbPath) {
    ...
    db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS pricing_catalog (
        ...
        input REAL NOT NULL,
        cache_create_5m REAL,
        cache_create_30m REAL,
        cache_create_1h REAL,
        cache_read REAL,
        output REAL NOT NULL,
        source_url TEXT NOT NULL,
        PRIMARY KEY(revision, row_id)
      );
      ...
      CREATE INDEX IF NOT EXISTS idx_telemetry_events_provider_time ON telemetry_events(provider, timestamp_ms);
    ...
    ensureSqliteConfiguration(db);
    return db;
  }

  function latestSqliteConfigurationRevision(db) {
    return db.prepare(`
  ...
  function configurationAtSqliteRevision(db, revision) {
    const settings = Object.fromEntries(db.prepare(`
      SELECT key, value_json
      FROM analytics_settings
      WHERE revision = ?
      ORDER BY key
    `).all(revision).map((row) => [row.key, JSON.parse(row.value_json)]));
    const prices = db.prepare(`
      SELECT *
      FROM pricing_catalog
      WHERE revision = ?
      ORDER BY provider, model, variant, row_id
    `).all(revision).map(sqlitePricingRow);
    return normalizeConfiguration({ revision, settings, prices });
  }

  function insertSqliteConfiguration(db, configuration, parentRevision = null, committedAtMs = Date.now()) {
    const normalized = normalizeConfiguration(configuration);
    const insertSetting = db.prepare(`
      INSERT INTO analytics_settings(revision, key, value_json) VALUES(?, ?, ?)
    `);
    for (const [key, value] of Object.entries(normalized.settings)) {
      insertSetting.run(normalized.revision, key, JSON.stringify(value));
    }
    ...
    `);
    for (const row of normalized.prices) {
      insertPrice.run(
        normalized.revision,
        row.id,
        row.provider,
    ...
    db.prepare(`
      INSERT INTO configuration_revisions(revision, parent_revision, committed_at_ms)
      VALUES(?, ?, ?)
    `).run(normalized.revision, parentRevision, committedAtMs);
    return normalized;
  }
  ...
  async function loadSqliteConfiguration(options = {}) {
    const db = openTokenomicsDatabase(resolveDbPath(options));
    try {
      return ensureSqliteConfiguration(db);
    } finally {
      db.close();
    }
  }

  async function saveSqliteConfiguration(options = {}, source = {}) {
    ...
    try {
      ...
      const currentConfiguration = configurationAtSqliteRevision(db, current.revision);
      ...
      const saved = insertSqliteConfiguration(db, next, current.revision, Math.max(Date.now(), number(current.committed_at_ms) + 1));
      db.exec("COMMIT");
      return saved;
    } catch (error) {
      db.exec("ROLLBACK");
  ...
  function deleteSourceRows(db, sourcePath) {
    ...
    db.prepare("DELETE FROM telemetry_events WHERE source_path = ?").run(sourcePath);
    ...
    return {
      ...
      insertTelemetry: db.prepare(`
        INSERT INTO telemetry_events(
          source_path, line_no, timestamp, timestamp_ms, provider, agent,
          model, project, event_kind, raw_json
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `),
    };
  }
  ...
  function insertTelemetryEventRow(statement, event, defaultSourcePath) {
    statement.run(
      event.sourcePath || defaultSourcePath,
      event.lineNo,
      event.timestamp,
  ...
  async function processAndStoreSource(db, source, fingerprint, options, sourceHeaders) {
    ...
    return withAsyncTransaction(db, async () => {
      ...
      report._telemetryEventSink = (event) => insertTelemetryEventRow(statements.insertTelemetry, event, source.path);
  ...
  async function syncJsonlSource(db, input, options, currentHeaders) {
    ...
    if (sameSourceFingerprint(existingSourceFingerprint(db, input.path), fingerprint)) return false;
  ...
  async function syncZipSource(db, input, options, limiter, currentHeaders) {
    ...
    for (const entry of entries) {
      ...
      if (sameSourceFingerprint(existingSourceFingerprint(db, sourcePath), fingerprint)) continue;
  ...
  async function syncSqliteDatabase(options) {
    ...
    try {
      const configuration = ensureSqliteConfiguration(db);
      ...
      const report = buildReportFromOpenDatabase(db, configuredOptions);
      attachSqliteConfigurationState(db, report, configuration);
  ...
  function addStoredUsage(report, row, options) {
    ...
    const quarterHour = quarterHourKey(timestamp);
    if (quarterHour) {
      addToStats(bucket(report.quarterHourly, quarterHour), usage, cost, visibleChars);
      addToStats(nestedBucket(report.projectQuarterHourly, project, quarterHour), usage, cost, visibleChars);
      report.quarterHourlyProviderModels[quarterHour] ??= {};
      addToStats(nestedBucket(report.quarterHourlyProviderModels[quarterHour], provider, model), usage, cost, visibleChars);
      report.projectQuarterHourlyProviderModels[project] ??= {};
      report.projectQuarterHourlyProviderModels[project][quarterHour] ??= {};
      addToStats(nestedBucket(report.projectQuarterHourlyProviderModels[project][quarterHour], provider, model), usage, cost, visibleChars);
    }
    ...
    addToStats(nestedBucket(report.projectProviderModels[project], provider, model), usage, cost, visibleChars);
  ...
  function storedRateLimitCurrent(row, options) {
    const timestamp = new Date(number(row.timestamp_ms));
    const usage = {
      input: number(row.input),
      cacheCreate5m: 0,
      cacheCreate30m: 0,
    ...
    return {
      ...
      model,
      usage,
      cost: calculateCost(inferProvider(model), model, usage, timestamp, options),
    };
  }
  ...
  function finalizeStoredRateLimits(db, report, options) {
    ...
    for (const row of db.prepare("SELECT * FROM rate_limit_samples ORDER BY group_key, timestamp_ms, sequence, id").iterate()) {
      const current = storedRateLimitCurrent(row, options);
  ...
  function buildReportFromOpenDatabase(db, options = {}) {
    ...
    for (const row of db.prepare("SELECT * FROM usage_events ORDER BY timestamp, id").iterate()) {
      addStoredUsage(report, row, options);
    }
    ...
    for (const row of db.prepare("SELECT timestamp, provider, agent, model, project, raw_json FROM telemetry_events WHERE event_kind = 'rate_limit_error' ORDER BY timestamp_ms, id").iterate()) {
      let payload = {};
    ...
    finalizeStoredRateLimits(db, report, options);
    return report;
  }
  ...
  function buildReportFromDatabase(dbPath, options = {}) {
    ...
    try {
      const configuration = ensureSqliteConfiguration(db);
      const report = buildReportFromOpenDatabase(db, pricingOptionsFromConfiguration(options, configuration));
      attachSqliteConfigurationState(db, report, configuration);
      return report;
    } finally {
      db.close();
  ...
  function attachSqliteConfigurationState(db, report, configuration) {
  ...
  return {
    ...
    loadConfiguration: loadSqliteConfiguration,
    openTokenomicsDatabase,
    resolveDbPath,
    saveConfiguration: saveSqliteConfiguration,
