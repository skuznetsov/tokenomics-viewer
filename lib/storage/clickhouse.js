...
function createClickHouseBackend(dependencies = {}) {
  ...
  async function storeClickHouseCodexSessionHeaders(client, headers, options = {}) {
    ...
    await clickHouseInsertRows(client, "codex_sessions", rows, options);
  }
  ...
  async function processAndStoreClickHouseSource(client, source, fingerprint, options) {
    // The source row is the completion marker. Remove any rows from a prior
    // changed or unmarked attempt before rebuilding the source by its stable
    // source_path key.
    await deleteClickHouseSourceRows(client, source.path);
    ...
    await clickHouseInsertRows(client, "sessions", report.sessions.map(clickHouseSessionRow), options);
    // Insert the marker only after every source-owned table has been written.
    await clickHouseInsertRows(client, "sources", [clickHouseSourceRow(source, fingerprint)], options);
    return report;
  }
  ...
  async function deleteClickHouseSourceRows(client, sourcePath) {
    for (const table of ["usage_events", "output_char_metrics", "rate_limit_samples", "sessions", "codex_sessions", "sources"]) {
      await clickHouseRequest(
        client,
  ...
  async function syncClickHouseJsonlSource(client, input, options) {
    ...
    await processAndStoreClickHouseSource(client, source, fingerprint, options);
    return true;
  }
